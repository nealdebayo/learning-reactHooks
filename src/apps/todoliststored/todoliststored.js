...
const IntroPage = () => {
	...
	useEffect(() => {
		setList(JSON.parse(localStorage.getItem('newtodos')) || [])
	})
	...
	const handleSubmit = event => {
		if (event.key==="Enter") {
			localStorage.setItem('newtodos', JSON.stringify([...list, { id: Date.now(), text: entry, completed: false }]))
			setEntry("")		
		}
	}
	...
	const handleDelete = id => {
		...
		setList(newList)
		localStorage.setItem('newtodos', JSON.stringify(newList))
	}
	...
	const theRealList = () => {
		if (list) {
			return list.map(l => <div className="blue darken-2" 
